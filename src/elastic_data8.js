...
function liveFeedToLogstash(program) {
  ...
  var client = restify.createJsonClient({ url: uri, rejectUnauthorized: false });
  client.basicAuth('elastic', 'Ta4np-j7Dt9A7vyvp2=2');
  var data = {
    derivative: 0,
  };
  ...
  function randomWalk(name, tags, start, variation, recoveryCallback) {
    ...
    client.post('/metrics-' + moment().format('YYYY.MM.DD') + '/_doc', message, function(err) {
      if (err) {
  ...
  function writeLogEntry(recoveryCallback) {
    ...
    client.post('/logs-' + moment().format('YYYY.MM.DD') + '/_doc', message, function(err) {
      if (err) {
